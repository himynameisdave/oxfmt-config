...
1. Go to the [package settings](https://www.npmjs.com/package/@himynameisdave/oxfmt-config/access) → **Trusted Publisher**.
2. Select **GitHub Actions** and enter:
   - Organization or user: `himynameisdave`
   - Repository: `oxfmt-config`
...
> The package doesn't exist on npm until its first publish, and trusted publishing can only be configured on an existing package. So the very first release is manual: see [Manual fallback](#manual-fallback-actions-unavailable), then come back and set up trusted publishing.
...
1. Create a [fine-grained PAT](https://github.com/settings/personal-access-tokens/new): resource owner `himynameisdave`, repository access limited to `oxfmt-config`, and **Contents: read & write** (nothing else).
2. Add it as the repository secret [`RELEASE_TOKEN`](https://github.com/himynameisdave/oxfmt-config/settings/secrets/actions).
...
- **patch**: comment fixes, README/docs, tooling that doesn't change formatted output
- **minor**: formatting decisions of any kind. An option flipped, a new option decided (e.g. after an oxfmt upgrade via the `update-oxfmt-options` skill), an override added or changed. All of these can reformat a consumer's files, and that's the deal `^` buys them.
- **major**: structural changes. An oxfmt major bump, an entry point renamed or removed, anything that makes consumers edit their `oxfmt.config.ts`.
...
2. **Gates.** `bun run test` — all four: `lint` (tsc build + oxlint with `@himynameisdave/oxlint-config`, type-aware on), `format:check` (oxfmt with this very config), `check-coverage` (every option in oxfmt's schema has an explicit decision), and `test:consumer` (pack, install and format from a scratch consumer project). A red gate aborts the release before anything is versioned or published.
3. **Version + tag.** `npm version <bump>` commits `🔖 Release vX.Y.Z` and tags `vX.Y.Z` (on the runner only, so far).
4. **Publish.** `prepublishOnly` runs the build (`tsc` → `dist/`), then npm uploads the tarball with provenance (9 files: two per entry point in `dist/` (`.js` + `.d.ts`, three entry points) plus LICENSE, README and package.json; `src/` never ships). Adding an entry point adds two files: check with `npm publish --dry-run` and update this count.
5. **Push + release.** The version commit and tag are pushed to `main`, and a GitHub release is created with auto-generated notes. Edit it afterwards to add highlights (which options changed, expected churn) if warranted.
...
npm view @himynameisdave/oxfmt-config version dist.fileCount

# it actually resolves and formats from a consumer:
cd "$(mktemp -d)" && bun init -y > /dev/null && bun add -D oxfmt @himynameisdave/oxfmt-config
```

Sanity-check the [npm page](https://www.npmjs.com/package/@himynameisdave/oxfmt-config) renders the README, and that the [FOSSA badges](https://app.fossa.com/projects/git%2Bgithub.com%2Fhimynameisdave%2Foxfmt-config) go green once FOSSA rescans.
...
- **Deprecating a version:** `npm deprecate @himynameisdave/oxfmt-config@"1.2.3" "reason"` steers installs away without breaking existing ones.
